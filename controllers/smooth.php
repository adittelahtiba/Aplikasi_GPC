...
function modus($arr)
{
	...
	for ($k = 0; $k < count($banyak); $k++) {
		if (max($banyak) == $banyak[$k]) {
			$modus = $angka[$k];
		}
	}
	return $modus;
...
for ($y = 0; $y < $height; $y++) {
	for ($x = 0; $x < $width; $x++) {
		...
		} else if ($direction == 'modus') {
			sort($arrr);
			sort($arrg);
			sort($arrb);
			$smoothedr[$x][$y] = modus($arrr);
			$smoothedg[$x][$y] = modus($arrg);
			$smoothedb[$x][$y] = modus($arrb);
		} else {
			sort($arrr);
			sort($arrg);
			sort($arrb);
